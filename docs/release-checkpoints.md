# Leftly Release Checkpoints

Use this file for small release checkpoint commits that intentionally trigger a Netlify deploy after several `[skip netlify]` polish commits.

## 2026-07-13 — Beta readiness guidance

Release checkpoint for recent polish work:

- Beta privacy and feedback guidance in Help / About / Feedback.
- Data screen copy clarifying local data, JSON backup, CSV export, sample numbers, and optional cloud backup.
- Copy-only beta readiness update after setup draft release.

This checkpoint intentionally does not include `[skip netlify]` so Netlify can deploy the latest `main` branch.

## 2026-07-12 — Setup draft polish

Release checkpoint for recent polish work:

- Setup draft saves first-paycheck setup progress when leaving setup.
- Close and Restart setup draft controls are separated.
- Setup draft release QA pass verified close, restart, completion, stale draft, and mobile behavior.

This checkpoint intentionally does not include `[skip netlify]` so Netlify can deploy the latest `main` branch.

## 2026-07-12 — Mobile and PWA polish

Release checkpoint for recent polish work:

- Mobile comfort pass for core flows.
- PWA installability basics.
- Manifest, metadata, and optional browser-install Help/About copy.

This checkpoint intentionally does not include `[skip netlify]` so Netlify can deploy the latest `main` branch.

## 2026-07-12 — Landing and wording polish

Release checkpoint for recent polish work:

- First-time landing screen without requiring login.
- Internal tester scenarios and QA guide.
- First-time user wording and confusing-label audit.

This checkpoint intentionally does not include `[skip netlify]` so Netlify can deploy the latest `main` branch.
